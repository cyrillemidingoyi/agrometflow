from agrometflow.indicators import process_indicators
...
def run_pipeline(config):
    ...
    if "climate" in config:
        ...
        downloader_source = source

        if source == "jasmin_http":
            downloader_source = "tamsat"
            
        downloader = get_climate_source(downloader_source, product=product,log_file=log_file, verbose=verbose)
        logger.info(f"climate_cfg: {climate_cfg}")
        download_result = downloader.download(**climate_cfg)
        if "points" in climate_cfg: results["climate"] = download_result
    ...
    if "indicators" in config:
        ...
        results["indicators"] = process_indicators(
